    ...
    public class HoSoesController : Controller
    {
        ...
        public ActionResult ImportData(int id)
        {
            ...
            var owners = hoso.HoSoPersons
                .Where(hp => hp.Role == "Owner")
                .Select(hp => hp.PersonInfo)
                .ToList();

            var buyersOrSellers = hoso.HoSoPersons
                .Where(hp => hp.Role == "BuyerOrSeller" || hp.Role == "Buyer" || hp.Role == "Seller")
                .Select(hp => hp.PersonInfo)
                .ToList();
            ...
            var viewModel = new InfomationEditViewModel
            {
                ...
                LandParcel = hoso.LandParcel,
                VariationInfo= hoso.VariationInfo
            };
        ...
        public ActionResult ImportData(int id, InfomationEditViewModel model)
        {
            ...
            {
                var ownerVm = model.Owners[i];
                var ownerDb = hoso.HoSoPersons
                    .Where(hp => hp.Role == "Owner")
                    .Skip(i).FirstOrDefault();

                if (ownerDb != null)
                {
                    ownerDb.PersonInfo.FullName = ownerVm.FullName ?? "---";
                    ownerDb.PersonInfo.BirthDate = ownerVm.BirthDate;
                    ownerDb.PersonInfo.Gender = ownerVm.Gender ?? "---";
                    ownerDb.PersonInfo.Hamlet = ownerVm.Hamlet ?? "---";
                    ownerDb.PersonInfo.Ward = ownerVm.Ward ?? "---";
                    ownerDb.PersonInfo.Province = ownerVm.Province ?? "---";
                    ownerDb.PersonInfo.DocumentType = ownerVm.DocumentType ?? "---";
                    ownerDb.PersonInfo.DocumentNumber = ownerVm.DocumentNumber ?? "---";
                    ownerDb.PersonInfo.IssueDate = ownerVm.IssueDate;
                    ownerDb.PersonInfo.Issuer = ownerVm.Issuer ?? "---";
                    ownerDb.PersonInfo.TaxCode = ownerVm.TaxCode ?? "---";
                    ownerDb.PersonInfo.DeathDate = ownerVm.DeathDate;
                    ownerDb.PersonInfo.DeathDocument = ownerVm.DeathDocument ?? "---";
                    ownerDb.PersonInfo.HeirId = ownerVm.HeirId;
                }
            }

            // Update BuyersOrSellers
            for (int i = 0; i < model.BuyersOrSellers.Count; i++)
            {
                var buyerVm = model.BuyersOrSellers[i];
                var buyerDb = hoso.HoSoPersons
                    .Where(hp => hp.Role == "BuyerOrSeller" || hp.Role == "Buyer" || hp.Role == "Seller")
                    .Skip(i).FirstOrDefault();

                if (buyerDb != null)
                {
                    buyerDb.PersonInfo.FullName = buyerVm.FullName ?? "---";
                    buyerDb.PersonInfo.BirthDate = buyerVm.BirthDate;
                    buyerDb.PersonInfo.Gender = buyerVm.Gender ?? "---";
                    buyerDb.PersonInfo.Hamlet = buyerVm.Hamlet ?? "---";
                    buyerDb.PersonInfo.Ward = buyerVm.Ward ?? "---";
                    buyerDb.PersonInfo.Province = buyerVm.Province ?? "---";
                    buyerDb.PersonInfo.DocumentType = buyerVm.DocumentType ?? "---";
                    buyerDb.PersonInfo.DocumentNumber = buyerVm.DocumentNumber ?? "---";
                    buyerDb.PersonInfo.IssueDate = buyerVm.IssueDate;
                    buyerDb.PersonInfo.Issuer = buyerVm.Issuer ?? "---";
                    buyerDb.PersonInfo.TaxCode = buyerVm.TaxCode ?? "---";
                    buyerDb.PersonInfo.DeathDate = buyerVm.DeathDate;
                    buyerDb.PersonInfo.DeathDocument = buyerVm.DeathDocument ?? "---";
                    buyerDb.PersonInfo.HeirId = buyerVm.HeirId;
                }
            }

            // Update LandParcel
            if (model.LandParcel != null && hoso.LandParcel != null)
            {
                ...
                hoso.LandParcel.Address = model.LandParcel.Address ?? null;
            ...
            // Update VariationInfos
            var varDb = hoso.VariationInfo;
            var varVm = model.VariationInfo;
            if (varDb != null && varVm != null)
            {
                varDb.VariationType = varVm.VariationType ?? "---";
                varDb.ContractNumber = varVm.ContractNumber ?? "---";
                varDb.NotaryOffice = varVm.NotaryOffice ?? "---";
                varDb.NotaryDate = varVm.NotaryDate;
                varDb.ContractAmount = varVm.ContractAmount;
                varDb.TaxReductionReason = varVm.TaxReductionReason ?? "---";
                varDb.LandPosition = varVm.LandPosition ?? "---";
            }           
